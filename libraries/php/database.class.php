...
{
/**
 * Comme en php 4 on ne peut pas faire des fonction et des methodes statiques,
 * on triche avec une fonction.
 * 
 * en PHP5 : static $s_hConnection = null;
 * 
 * @author Lionel SAURON
 * @private
 * 
 * @param $hConnection(connection_handle) (Optionel) Identifiant de connexion.
 * @return (connection_handle) Identifiant de connexion.
 */
function cheaterConnection($hConnection = null)
{
	static $s_hConnection = null;
	
	if($hConnection === null)
	{
		// Remplace la lecture de self::$hConnection du php5
		return $s_hConnection;
	}
	else
	{
		// Remplace l'ecriture dans self::$hConnection du php5
		$s_hConnection = $hConnection;
	}
}

/**
 * Comme en php 4 on ne peut pas faire des fonctions et des methodes statiques,
 * on triche avec une fonction.
 * 
 * en PHP5 : static $s_sErrorHandler = array("Database", "defaultErrorHandler");
 * 
 * @author Lionel SAURON
 * @private
 * 
 * @param $sErrorHandler(string) (Optionel) Methode du gestionnaire d'erreur.
 * @return (string) Methode du gestionnaire d'erreur.
 */
function cheaterErrorHandler($sErrorHandler = null)
{
	static $s_sErrorHandler = array("Database", "defaultErrorHandler");
	
	if($sErrorHandler === null)
	{
		// Remplace la lecture de self::$aError du php5
		return $s_sErrorHandler;
	}
	else
	{
		// Remplace l'ecriture dans self::$aError du php5
		$s_sErrorHandler = $sErrorHandler;
	}
}		
 ...
 */
function setErrorHandler($sErrorHandler)
{
	Database::cheaterErrorHandler($sErrorHandler);
}
 ...
 */
function defaultErrorHandler($aError)
{
 ...
 */
function sendError($hResource, $sSqlQuery)
{
	if($hResource === null)
	{
	...
	$sFunctionName = Database::cheaterErrorHandler();
 ...
 */
function openConnection($sLogin, $sPassword, $sDatabaseName, $sHost)
{	
	// Connexion au serveur
	$hConnection = @mysql_connect($sHost, $sLogin, $sPassword);
	if($hConnection === false)
	{
		Database::sendError(null, "mysql_connect");
		return false;
	}
	
	Database::cheaterConnection($hConnection);
	
	$bOk = @mysql_select_db($sDatabaseName, $hConnection);
	if($bOk == false)
	{
		Database::sendError($hConnection, "mysql_select_db");
		return false;
	}
	return true;
}
 ...
 * @return <code>true</code> ou <code>false</code>.
 */
function closeConnection($hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
		$hConnection = Database::cheaterConnection();
	}
	
	$bOk = @mysql_close($hConnection);
	if($bOk == false)
	{
		Database::sendError($hConnection, "mysql_close");
		return false;
	}
 ...
 */
function prepareString($sText)
{
	if($sText === null) return "null";
	if(strtoupper(trim($sText)) == "NULL") return $sText;
	
	$sText = "'".Database::escapeString($sText)."'";
 ...
 */
function prepareAndExecuteQuery($sSQLQuery, $hConnection)
{
	$hStatement = @mysql_query($sSQLQuery, $hConnection);
	if($hStatement === false)
	{
		Database::sendError($hConnection, $sSQLQuery);
		return false;
	}
 ...
 */
function execute($sSQLQuery, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
	...
	if($nNbRow === false)
	{
		Database::sendError($hConnection, "mysql_affected_rows");
		return false;
	}
 ...
 */
function fetchArray($sSQLQuery, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
	...
	$hStatement = Database::prepareAndExecuteQuery($sSQLQuery, $hConnection);
 ...
 * @return <code>false</code> ou le tableau de données lues.
 */
function fetchArrayWithKey($sSQLQuery, $sKey, $bUnique = true, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
 ...
 * @return <code>false</code> ou le tableau de données lues.
 */
function fetchArrayWithMultiKey($sSQLQuery, $aKeys, $bUnique = true, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
 ...
 */
function fetchColumn($sSQLQuery, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
 ...
 * @return <code>false</code> ou le tableau de donnée lue.
 */
function fetchColumnWithKey($sSQLQuery, $bUnique = true, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
 ...
 * @return <code>false</code> ou le tableau de donnée lue.
 */
function fetchColumnWithMultiKey($sSQLQuery, $bUnique = true, $hConnection = null)
{
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
 ...
 */
function fetchOneRow($sSQLQuery, $hConnection = null)
{	
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 
 ...
 */
function fetchOneValue($sSQLQuery, $hConnection = null)
{	
	if($hConnection === null)
	{
		// Cheater pour les membres static en php 4 

...
class FormValidation
{
	...
	function initCurrentRef($sInputName, $bCreateData = false)
	{
		...
		if(($bInitDataRef == true) || ($bCreateData == true))
		{
			...
			{
				if((is_array($vPointer) == false)
				|| (array_key_exists($sOneSubInputName, $vPointer) == false))
				{
	 ...
	 * - is_bool        = teste si le type du champ est boolean.
	 * - convert_bool   = teste si le type du champ est boolean (stocke la valeur convertie).
	 *
	...
	function test0($sInputName, $sTestName)
	{
		...
		switch(strtoupper($sTestName))
		{
			...
			case 'IS_BOOL':
				...
				// On enleve les espace après car sinon il ne peut être considéré comme un int par is_numeric.
				$Value = trim($Value);
			...
			case 'CONVERT_BOOL':
				...
				$this->paCurrentInfo['DATA'] = ($Value == 0) ? false : true;
			break;
	 ...
	 * - min_length      = teste si la longeur de la valeur du champ est supérieur ou égale à <code>$Param1</code>.
	 * - max_length      = teste si la longeur de la valeur du champ est inférieur ou égale à <code>$Param1</code>.
	 * - in_list         = teste si la valeur du champ est partie les valeurs de la liste <code>$Param1</code>.
	 *
	 * Renvoie <code>null</code> si les tests ont été stoppés avant
	 * ou si le nom du test n'existe pas.
	 *
	...
	function test1($sInputName, $sTestName, $Param1)
	{
		...
		switch(strtoupper($sTestName))
		{
			...
			// ----------
			case 'MIN_LENGTH':
				if(is_string($Value)  == false) break;
				if(is_numeric($Param1) == false) break;

				$bTestOk = ($Param1 <= strlen($Value)) ? true : false;
			break;
			...
			case 'MAX_LENGTH':
				...
				$bTestOk = (strlen($Value) <= $Param1) ? true : false;
			break;

			// ----------
			case 'IN_LIST':
				if(is_array($Param1) == false) break;

				$bTestOk = (in_array($Value, $Param1, true)) ? true : false;
			break;

...
$bHasRight = ProfilManager::hasRight('eval_ind_add');
if($bHasRight == false)
{
	// Redirection
...
____EOQ;
	...
	$sQuery = <<< ____EOQ
		SELECT
			...
			COMPETENCE_NOM
		FROM EVALUATIONS_COLLECTIVES
			INNER JOIN EVAL_COMPETENCES
				ON EVALUATIONS_COLLECTIVES.EVAL_COL_ID = EVAL_COMPETENCES.ID_EVAL_COL
			INNER JOIN COMPETENCES
				ON EVAL_COMPETENCES.ID_COMPETENCE = COMPETENCES.COMPETENCE_ID
            INNER JOIN MATIERES
...
echo h1("Ajout d'une &eacute;valuation individuelle", $aObjectsToHide);
?>
